    ...
    if isinstance(kwargs_user, dict):
        ...
        if i is not None:
            if i < len(kwargs_user) and kwargs_user[i] is not None:
                return (
                    kwargs | kwargs_user[i]
                )  # pyright: ignore[reportOperatorIssue]  # this is correct, but pyright cannot narrow the types of list elements
            else:
                return kwargs
        else:
            raise ValueError("i must be given if kwargs_user is a list")
